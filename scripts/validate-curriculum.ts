...
assert.equal(regions.length, 13, "Grade 8 must contain 13 regions");
assert.equal(lessons.length, 52, "Grade 8 must contain 52 lessons");
assert.equal(curriculumStats.questions, 260, "Every lesson must contain five reviewed questions");
...
console.log(`Validated ${lessons.length} lessons, ${curriculumStats.questions} questions, and ${propertyChecks} seeded answer checks.`);
