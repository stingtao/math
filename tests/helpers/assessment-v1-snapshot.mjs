...
import { depthPackBySlug, getQuestionBank } from "../../lib/curriculum-depth.ts";
...
// Hash complete serialized questions, including prompts, answers, interaction
// configuration, hints, explanations, and evidence. Object-key order is immaterial.
...
export function captureV1Snapshot(seeds, packSlugs) {
  const version = "depth-v1";
  ...
  const packs = Object.fromEntries(packSlugs.map((slug) => [slug, digest(depthPackBySlug.get(slug))]));
