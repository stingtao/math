...
const contextScenes: Record<string, ContextScene> = {
  ...
  "g7-percent-decision-chains": {
    src: "/visuals/percent-market-context.webp",
    alt: "Two market offers show a discount, a fee, and tax applied in a different order",
    headline: "each percent uses the amount at that step",
    copy: "Apply the discount first, use the new price for tax, then compare final costs.",
    model: "percent",
    mathSteps: ["\\$100", "20\\%\\text{ off}\\Rightarrow\\$80", "8\\%\\text{ tax}\\Rightarrow\\$6.40", "\\$86.40"],
    mathStepLabels: ["original", "sale price", "tax on sale price", "final"],
    modelLabel: "Math model: one hundred dollars with twenty percent off becomes eighty dollars; eight percent tax on eighty is six dollars forty, so the final price is eighty-six dollars forty",
  },
  "g8-composed-transformations": {
    src: "/visuals/transform-plaza-context.webp",
    alt: "A geometric plaza shows one figure reflected and then shifted to a final location",
    headline: "the first image becomes the next input",
    copy: "Apply one transformation completely, record the new coordinates, then use the next rule.",
    model: "transform",
    mathSteps: ["(2,1)", "\\text{reflect over }y\\text{-axis}", "(-2,1)", "\\text{right }5", "(3,1)"],
    mathStepLabels: ["original", "first rule", "image", "second rule", "final"],
    modelLabel: "Math model: reflect two comma one over the y-axis to negative two comma one, then translate right five to three comma one",
  },
};
...
function CoverageConceptVisual({ lesson }: { lesson: LessonDefinition }) {
  if (lesson.slug === "g9-quantities-units-precision") return <div className="lesson-visual coverage-concept-visual quantities-precision-visual" role="img" aria-label="Distance 12.4 kilometers divided by time 0.5 hours gives speed 24.8 kilometers per hour; a temperature of 18.37 degrees rounds to 18.4 degrees at the requested precision"><div className="coverage-metric-row"><span><small>DISTANCE</small><strong>12.4 km</strong></span><i>÷</i><span><small>TIME</small><strong>0.5 h</strong></span><i>=</i><span className="result"><small>SPEED</small><strong>24.8 km/h</strong></span></div><div className="precision-strip"><span>sensor: 18.37°C</span><b>nearest tenth →</b><strong>18.4°C</strong></div></div>;
  if (lesson.slug === "g9-absolute-value-functions") return <div className="lesson-visual coverage-concept-visual"><svg className="coverage-graph" viewBox="0 0 520 280" role="img" aria-label="V-shaped graph of y equals absolute value of x minus 3, minus 2, with vertex at 3 negative 2"><g className="coverage-grid">{[80,140,200,260,320,380,440].map((x) => <line x1={x} x2={x} y1="25" y2="245" key={`x${x}`} />)}{[45,95,145,195,245].map((y) => <line x1="45" x2="475" y1={y} y2={y} key={`y${y}`} />)}</g><line className="coverage-axis" x1="45" x2="475" y1="175" y2="175" /><line className="coverage-axis" x1="140" x2="140" y1="25" y2="245" /><path className="absolute-path" d="M80 125 L320 225 L455 169" /><circle className="coverage-focus-point" cx="320" cy="225" r="7" /><text x="332" y="218">vertex (3, −2)</text></svg><strong>y = |x − 3| − 2</strong><p>The vertex records the horizontal and vertical shifts.</p></div>;
  if (lesson.slug === "g9-absolute-value-inequalities") return <div className="lesson-visual coverage-concept-visual"><svg className="coverage-number-line" viewBox="0 0 520 170" role="img" aria-label="Open interval from negative 2 to 8 representing the solutions to absolute value of x minus 3 less than 5"><line className="coverage-axis" x1="55" x2="465" y1="85" y2="85" /><line className="coverage-solution" x1="145" x2="385" y1="85" y2="85" /><circle cx="145" cy="85" r="9" /><circle cx="385" cy="85" r="9" /><text x="128" y="125">−2</text><text x="378" y="125">8</text><text className="coverage-equation" x="156" y="48">|x − 3| &lt; 5</text></svg><strong>−2 &lt; x &lt; 8</strong><p>“Less than” keeps every point within 5 units of 3.</p></div>;
  if (lesson.slug === "g9-graph-linear-inequalities") return <div className="lesson-visual coverage-concept-visual"><svg className="coverage-graph" viewBox="0 0 520 280" role="img" aria-label="The inequality y greater than 2x minus 1 shown as a dashed boundary line with the half-plane above it shaded"><g className="coverage-grid">{[80,140,200,260,320,380,440].map((x) => <line x1={x} x2={x} y1="25" y2="245" key={`x${x}`} />)}{[45,95,145,195,245].map((y) => <line x1="45" x2="475" y1={y} y2={y} key={`y${y}`} />)}</g><polygon className="inequality-shade" points="45,25 475,25 475,75 90,245 45,245" /><line className="coverage-axis" x1="45" x2="475" y1="145" y2="145" /><line className="coverage-axis" x1="260" x2="260" y1="25" y2="245" /><line className="inequality-boundary" x1="90" x2="475" y1="245" y2="75" /><text x="310" y="58">shade above</text></svg><strong>y &gt; 2x − 1</strong><p>Strict inequality → dashed boundary. Greater y-values → shade above.</p></div>;
  if (lesson.slug === "g9-systems-linear-inequalities") return <div className="lesson-visual coverage-concept-visual"><svg className="coverage-graph" viewBox="0 0 520 280" role="img" aria-label="Triangular feasible region satisfying x at least zero, y at least zero, and x plus y at most 6"><g className="coverage-grid">{[80,140,200,260,320,380,440].map((x) => <line x1={x} x2={x} y1="25" y2="245" key={`x${x}`} />)}{[45,95,145,195,245].map((y) => <line x1="45" x2="475" y1={y} y2={y} key={`y${y}`} />)}</g><line className="coverage-axis" x1="80" x2="475" y1="225" y2="225" /><line className="coverage-axis" x1="80" x2="80" y1="25" y2="245" /><polygon className="system-shade" points="80,225 80,55 390,225" /><line className="system-boundary" x1="80" x2="390" y1="55" y2="225" /><text x="145" y="185">all 3 rules overlap</text></svg><strong>x ≥ 0 · y ≥ 0 · x + y ≤ 6</strong><p>The shared region contains every viable solution.</p></div>;
  if (lesson.slug === "g9-linear-quadratic-systems") return <div className="lesson-visual coverage-concept-visual"><svg className="coverage-graph" viewBox="0 0 520 280" role="img" aria-label="The line y equals x plus 2 crosses the parabola y equals x squared at negative one comma one and two comma four"><g className="coverage-grid">{[80,140,200,260,320,380,440].map((x) => <line x1={x} x2={x} y1="25" y2="245" key={`x${x}`} />)}{[45,95,145,195,245].map((y) => <line x1="45" x2="475" y1={y} y2={y} key={`y${y}`} />)}</g><line className="coverage-axis" x1="45" x2="475" y1="220" y2="220" /><line className="coverage-axis" x1="220" x2="220" y1="25" y2="245" /><path className="quadratic-model-path" d="M70 35 Q220 365 430 35" /><line className="linear-model-fit" x1="70" x2="450" y1="225" y2="55" /><circle className="coverage-focus-point" cx="160" cy="185" r="7" /><circle className="coverage-focus-point vertex" cx="340" cy="105" r="7" /><text x="84" y="48">y = x²</text><text x="350" y="72">y = x + 2</text></svg><strong>x² = x + 2 → x = −1 or 2</strong><p>Each intersection satisfies the line and the parabola.</p></div>;
  if (lesson.slug === "g9-build-quadratic-models") return <div className="lesson-visual coverage-concept-visual"><svg className="coverage-graph" viewBox="0 0 520 280" role="img" aria-label="Downward parabola with zeros at 1 and 5 and vertex at 3 comma 8, modeled by negative 2 times x minus 1 times x minus 5"><g className="coverage-grid">{[80,140,200,260,320,380,440].map((x) => <line x1={x} x2={x} y1="25" y2="245" key={`x${x}`} />)}{[45,95,145,195,245].map((y) => <line x1="45" x2="475" y1={y} y2={y} key={`y${y}`} />)}</g><line className="coverage-axis" x1="45" x2="475" y1="220" y2="220" /><path className="quadratic-model-path" d="M120 322 Q260 -212 400 322" /><circle className="coverage-focus-point" cx="150" cy="220" r="7" /><circle className="coverage-focus-point" cx="370" cy="220" r="7" /><circle className="coverage-focus-point vertex" cx="260" cy="55" r="7" /><text x="132" y="250">1</text><text x="365" y="250">5</text><text x="275" y="54">(3, 8)</text></svg><strong>y = −2(x − 1)(x − 5)</strong><p>Zeros choose the factors; the known point determines the scale.</p></div>;
  if (lesson.slug === "g9-interpret-linear-models") return <div className="lesson-visual coverage-concept-visual"><svg className="coverage-graph" viewBox="0 0 520 280" role="img" aria-label="Observed fuel data near the line y equals 6x plus 20; slope 6 means six fuel units per hour and intercept 20 is the predicted starting value"><g className="coverage-grid">{[80,140,200,260,320,380,440].map((x) => <line x1={x} x2={x} y1="25" y2="245" key={`x${x}`} />)}{[45,95,145,195,245].map((y) => <line x1="45" x2="475" y1={y} y2={y} key={`y${y}`} />)}</g><line className="coverage-axis" x1="45" x2="475" y1="225" y2="225" /><line className="coverage-axis" x1="70" x2="70" y1="25" y2="245" /><line className="linear-model-fit" x1="70" x2="410" y1="206" y2="54" /><line className="linear-model-extrapolation" x1="410" x2="475" y1="54" y2="25" />{[[120,190],[165,170],[220,145],[278,127],[334,90],[386,73]].map(([x,y]) => <circle className="linear-model-point" cx={x} cy={y} r="6" key={`${x}-${y}`} />)}<text x="86" y="198">20 at x=0</text><text x="265" y="92">+6 fuel/hour</text><text x="395" y="42">predict cautiously</text></svg><strong>y = 6x + 20</strong><p>Slope and intercept need story units; association does not prove causation.</p></div>;
  return null;
}
...
export function ConceptVisual({ lesson }: { lesson: LessonDefinition }) {
  ...
  const coverageVisual = <CoverageConceptVisual lesson={lesson} />;
  if (["g9-quantities-units-precision", "g9-absolute-value-functions", "g9-absolute-value-inequalities", "g9-graph-linear-inequalities", "g9-systems-linear-inequalities", "g9-linear-quadratic-systems", "g9-build-quadratic-models", "g9-interpret-linear-models"].includes(lesson.slug)) return coverageVisual;
