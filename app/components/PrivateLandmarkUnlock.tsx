...
export function PrivateLandmarkUnlock({
  ...
  return (
    <section className={`settlement-landmark accent-${achievement.tone} ${compact ? "compact-landmark" : ""}`} role="status" aria-label={`${achievement.title} achievement unlocked`}>
      ...
      <div><small>NEW ACHIEVEMENT</small><strong>{achievement.title}</strong>{!compact && <p>{achievement.copy}</p>}</div>
      {!compact && <a href={`/profile${demo ? "?demo=1" : ""}#achievement-heading`}>View achievements <span aria-hidden="true">→</span></a>}
    </section>
  );
}
