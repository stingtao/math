...
export function LeaderboardView({ demo }: { demo: boolean }) {
  ...
  return (
    <main className="site-shell leaderboard-page">
      {loading ? <header className="leaderboard-header-placeholder"><Brand /><span>Checking your private trail…</span></header> : state ? <LearnerHeader state={state} demo={demo} /> : <PublicHeader />}
      ...
      <section className="leaderboard-wrap">
        {viewerEntry && <div className="current-rank-card"><Avatar avatar={viewerEntry.avatar} size="md" label="Your anonymous league avatar" /><div><small>YOUR PRIVATE POINTER</small><strong>Rank {viewerEntry.rank} of {entries.length}</strong><p>This “You” marker is added only in your signed-in view. Other learners see only your random identity.</p></div><span><b>{viewerEntry.weeklyXp}</b> XP</span></div>}
