...
type Entry = { id: string; rank: number; nickname: string; avatar: AvatarSpec; weeklyXp: number; isViewer?: boolean };

export function LeaderboardView({ demo }: { demo: boolean }) {
  const { state, loading, isDemo } = useLearner(demo);
  ...
  const profileHref = isDemo ? "/profile?demo=1" : "/profile";

  return (
    <main className="site-shell leaderboard-page">
      {loading ? <header className="leaderboard-header-placeholder"><Brand /><span>Loading this week’s board…</span></header> : state ? <LearnerHeader state={state} demo={isDemo} /> : <PublicHeader />}
      <section className="leaderboard-hero">
        <div><span className="eyebrow">WEEKLY LEAGUE</span><h1>Race your week—not your identity.</h1><p>A fresh alias and XP race begin every Monday.</p></div>
      ...
      <section className="leaderboard-wrap">
        ...
        {state && !viewerEntry && <div className="league-view-note"><span aria-hidden="true">◇</span><div><strong>{isDemo ? "Previewing the public board" : "You are not shown on this board"}</strong><p>{isDemo ? "Demo progress stays on this device and is never posted." : "Your private codename is never used here. Joining creates a separate alias for this week."}</p></div><a href={profileHref}>{isDemo ? "Open demo profile" : "League privacy settings"} →</a></div>}
        {entries.length > 0 && <div className="podium">
          {[entries[1], entries[0], entries[2]].filter(Boolean).map((entry, index) => <article className={`podium-place podium-${index === 1 ? 1 : index === 0 ? 2 : 3} ${entry.isViewer ? "current-learner" : ""}`} key={entry.id}><span className="podium-rank">{entry.rank}</span><Avatar avatar={entry.avatar} size="lg" label={`${entry.nickname}'s abstract avatar`} /><strong>{entry.nickname}{entry.isViewer && <small>YOU</small>}</strong><span>{entry.weeklyXp} XP</span></article>)}
        </div>}
        <div className="leaderboard-card">
          ...
          <ol className="leaderboard-list">
            {entries.map((entry) => <li className={entry.isViewer ? "current-learner" : ""} key={entry.id}><span className={`rank-number rank-${entry.rank}`}>{entry.rank}</span><Avatar avatar={entry.avatar} size="sm" /><strong>{entry.nickname}{entry.isViewer && <small>YOU</small>}</strong><span className="xp-bar"><i style={{ width: `${entry.weeklyXp > 0 ? Math.max(12, entry.weeklyXp / maxXp * 100) : 0}%` }} /></span><b>{entry.weeklyXp} XP</b></li>)}
          </ol>
