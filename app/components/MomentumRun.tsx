...
export function MomentumRun({ label, current, best, total, tone, justLinked = false }: MomentumRunProps) {
  ...
  return (
    <div className={`momentum-run momentum-${tone} ${justLinked ? "just-linked" : ""}`} role="group" aria-live="off" aria-label={`${label}: current chain ${current}, best chain ${best}, out of ${total}`}>
      <span className="momentum-run-emblem" aria-hidden="true"><strong>×{current}</strong><small>CHAIN</small></span>
      ...
      <b>BEST ×{best}</b>
      <p>Bonus signal only—corrections still advance.</p>
    </div>
  );
}
