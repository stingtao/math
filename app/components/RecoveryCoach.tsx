...
export function RecoveryCoach({ question, response, failedAttempts, correctedCount, total, memoryCheck }: { question: PracticeQuestion; response: string; failedAttempts: number; correctedCount: number; total: number; memoryCheck: boolean }) {
  ...
  return (
    <div id="lesson-answer-feedback" className="recovery-coach" role="status" aria-live="polite">
      <header><span aria-hidden="true">↻</span><div><small>MISTAKE → METHOD · {guidance.label}</small><strong>{guidance.title}</strong><p>{guidance.explanation}</p></div></header>
      <div className="recovery-coach-steps">
        <article><b>1</b><div><small>NOTICE</small><strong>Find the one step to change</strong><p>{guidance.nextMove}</p></div></article>
        <i aria-hidden="true">→</i>
        <article><b>2</b><div><small>REBUILD</small><strong>Explain before calculating</strong><p>{guidance.selfCheck}</p></div></article>
        <i aria-hidden="true">→</i>
      ...
      <footer><span aria-hidden="true">◇</span><p><strong>No progress lost.</strong> A mistake creates your next practice step; it does not remove points you already earned.</p></footer>
    </div>
  );
}
