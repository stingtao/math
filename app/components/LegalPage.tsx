  ...
  return (
    <main className="site-shell legal-page">
      ...
      <article className="legal-wrap">
        <header><span className="eyebrow">PLAIN ENGLISH · VERSION 1.0</span><h1>{privacy ? "Privacy should be easy to understand." : "Simple rules for a calm learning space."}</h1><p>{privacy ? "Math is designed to remember learning progress without building a real-name student profile." : "These terms explain the boundaries of the Grade 8 learning service."}</p></header>
        {privacy ? <PrivacyCopy /> : <TermsCopy />}
        <footer><p>Last updated: August 20, 2026</p><a className="primary-button" href="/">Return home <span>→</span></a></footer>
      </article>
    </main>
  );
}
...
function PrivacyCopy() {
  return <div className="legal-sections">
    ...
    <section><span>03</span><div><h2>What Math saves</h2><p>Math saves lesson progress, answer correctness, hint use, review timing, stars, XP, rewards, boss clears, streaks, privacy preferences, and secure session records. Raw written answers are not retained after checking.</p></div></section>
    ...
    <section><span>05</span><div><h2>Analytics and advertising</h2><p>Math does not use advertising, third-party behavioral analytics, session replay, or data sales. Operational logs must exclude credentials, Google profile claims, raw account subjects, and answer text.</p></div></section>
    ...
    <section><span>07</span><div><h2>Age boundary</h2><p>The current service is intended for learners age 13 or older. It does not collect birth dates. A future version for younger learners will require a separate parental-consent and child-privacy design.</p></div></section>
  </div>;
}
...
function TermsCopy() {
  return <div className="legal-sections">
    ...
    <section><span>02</span><div><h2>Learning content</h2><p>The first release covers Grade 8 Common Core topics through short lessons, practice, review, and game-like challenges. Content is educational guidance and may not match every school’s local sequence.</p></div></section>
