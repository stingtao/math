...
The site includes 55 regions, 253 lessons, 1,346 reviewed practice questions, mixed boss checks, spaced review, daily rewards, anonymous identities, an opt-in weekly leaderboard, and a public feedback board whose posts are not connected to learner accounts or progress. Grade 8–12 now includes graph-choice, table-choice, coordinate, number-line, and multi-select reasoning missions across every region. A public Linear Graph Lab lets learners type `y = mx + b` equations and see the graph, slope, intercept, nearby coordinates, and point table update without signing in or saving the input. Its point-to-line mission turns plotted coordinates into a connected line and then asks learners to read coordinates back from the graph.
